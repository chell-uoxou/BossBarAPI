...
class API{
	...
	public static function addBossBar($players, string $title, $ticks = null){
		...
		$packet->yaw = 0;
		$packet->pitch = 0;
		$packet->metadata = [Entity::DATA_LEAD_HOLDER_EID => [Entity::DATA_TYPE_LONG, -1], Entity::DATA_FLAG_SILENT => [Entity::DATA_TYPE_BYTE, 1], Entity::DATA_SCALE => [Entity::DATA_TYPE_FLOAT, 0], Entity::DATA_NAMETAG => [Entity::DATA_TYPE_STRING, $title], 
	...
	public static function setPercentage(int $percentage, int $eid){
		...
		$upk->entries[] = new BossBarValues(0, 300, max(1, min([$percentage, 100])) / 100 * 300, 'minecraft:health'); // Ensures that the number is between 0 and 100;
		$upk->entityId = $eid;
